...
SIGNALING_SDK_VERSION = "1.2.0.804";
Signal_ = function(vid) {
	...
	signal.server_urls = [];
	signal.setup_debugging = function(name, value) {
		if (name === "ap") {
			signal.server_urls.push([value, 8001]);
			signal.debugging = true
		} else if (name === "env") {
			if (value === "lbs100") {
				signal.lbs_url1 = ["https://lbs100-1-sig.agora.io", "https://lbs100-2-sig.agora.io"];
				signal.lbs_url2 = ["https://lbs100-3-sig.agora.io", "https://lbs100-4-sig.agora.io"]
			}
		} else {
			return
		}
	...
	var Session = function(account, token) {
			...
			this.debugging = signal.debugging;
			this.m_msgid = 0;
			...
			var server_urls = signal.server_urls.slice();
			shuffle(server_urls);
			session.idx = 0;
			session.v3_msg_set = new Map;
			setTimeout(function() {
				now = Date.now();
				for (var key of session.v3_msg_set.keys()) {
					if (session.v3_msg_set[key]) {
						if (now - session.v3_msg_set[key] > 3e5) {
							session.v3_msg_set.delete(key)
						} else {
							break
						}
					}
			...
			session.logout = function() {
				if (session.state == LOGIN_STATE_LOGINED && session.onLogout) {
				...
				} else if (session.state == LOGIN_STATE_LOGINING) {
					session.state == LOGIN_STATE_NOT_LOGIN;
					session.fire_logout(LOGOUT_E_USER)
				}
			};
			session.fire_login_failed = function(reason) {
				try {
					if (session.state == LOGIN_STATE_LOGINING && session.onLoginFailed) {
			...
			var do_lbs = function(retry, urls, idx) {
					...
					var url = urls[idx];
					timedGetText(url + "/getaddr?vid=" + vid, 5e3, function(err, data) {
						if (err) {
							if (retry - 1 > 0) {
								do_lbs(retry - 1, urls, (idx + 1) % urls.length)
							} else {
								session.fire_login_failed(LOGIN_E_NET)
							}
						} else {
							if (session.lbs_state != "requesting") {
								return
			...
			var do_connect = function() {
					if (0) {
						...
						if (session.state == LOGIN_STATE_LOGINING) {
							var socket = new function() {
									var url = format_ws_url(server_urls[session.idx]);
									console.log(url);
									session.idx = (session.idx + 1) % server_urls.length;
									...
									websocket.onopen = function(evt) {
										if (session.state == LOGIN_STATE_NOT_LOGIN) {
											websocket.close()
										} else if (session.state == LOGIN_STATE_LOGINING && session.socket == null) {
									...
									websocket.onclose = function(evt) {
										if (websocket.state == "open" && session.state === LOGIN_STATE_LOGINED) {
											if (session.onLogout) {
												session.onLogout("WebSocket connection closed.")
											}
											fire("_close", "");
											dbg("on conn close")
										}
									...
									websocket.onerror = function(evt) {
										...
										dbg("on conn error");
										if (session.state == LOGIN_STATE_LOGINED && session.socket == socket) {
											session.fire_logout("conn error")
										} else if (session.state == LOGIN_STATE_LOGINING && session.socket == socket) {
											session.fire_login_failed(LOGIN_E_NET)
										}
									};
									var evts = {};
					...
					var start_ping = function() {
							setTimeout(function() {
								if (session.state != LOGIN_STATE_LOGINED) {
									return
								}
								ping_i++;
					...
					session.go_login = function() {
						if (session.line == "") {
							...
							session.socket.on("login_ret", function(x) {
								...
								if (!err && ret["result"] == "ok") {
									session.config_msg_set = ret["config_msg_set"] || 0;
									session.config_inst_msg_with_msgid = ret["config_inst_msg_with_msgid"] || 0;
									session.uid = ret["uid"];
									session.line = ret["line"];
									session.state = LOGIN_STATE_LOGINED;
									start_ping();
									start_tick();
									try {
									...
									try {
										if (session.onLoginFailed) {
											var e = err == "kick" ? LOGIN_E_TOKEN_KICKED : err == "TokenErrorExpired" ? LOGIN_E_TOKENEXPIRED : err.startsWith("TokenError") ? LOGIN_E_TOKENWRONG : LOGIN_E_NET;
						...
						var decode_msg = function(msg) {
								...
								} else if (msg.startsWith("msg-v3 ")) {
									var r = split_python(msg, " ", 7);
									if (r.length == 8) {
										if (session.v3_msg_set.get(r[1])) {
											return null
										}
										session.v3_msg_set.set(r[1], Date.now());
										var src = r[2];
										var t = r[5];
										var content = r[7];
										return [src, t, content]
						...
						var m_msgs_wait_clear = [];
						var m_time_poll_ack = 0;
						var schedule_poll = function() {
								if (m_is_polling) return;
								m_is_polling = true;
								m_time_poll_ack = 0;
								if (session.config_msg_set === 0) {
									session.call2("user_getmsg", {
										...
										if (err == "") {
											...
											m_ver_ack = parseInt(resp["ver_clear"]);
											m_ver_clear = Math.max(m_ver_ack, ver_clear_old);
								...
								} else if (session.config_msg_set === 1) {
									session.call2("user_getmsg2", {
										line: session.line,
										clear_msgs: m_msgs_wait_clear,
										max: 30
									}, function(err, data) {
										if (err === "") {
											m_msgs_wait_clear = [];
											for (var i in data["msgs"]) {
												var v = data["msgs"][i][0];
												var line = data["msgs"][i][1];
												process_msg(decode_msg(line))
											}
											if (data["msgs"].length >= 30) {
												schedule_poll()
											}
											m_last_active_time = get_time_in_ms()
										}
						...
						var schedule_poll_tail = function() {
								if (session.config_msg_set === 0) {
									...
									if (m_time_poll_ack === 0) {
										m_time_poll_ack = get_time_in_ms() + 500
									}
								}
							};
						var start_tick = function() {
								setTimeout(function() {
									...
									if (session.state == LOGIN_STATE_LOGINED) {
										...
										if (session.config_msg_set === 0) {
											if (m_ver_ack < m_ver_clear && t - m_time_poll_last > 1e3) {
												schedule_poll()
											} else if (t - m_time_poll_last >= 1e3 * 60) {
												schedule_poll()
											}
										} else if (session.config_msg_set === 1) {
											if (m_msgs_wait_clear.length > 0 && t > m_time_poll_ack && m_time_poll_ack > 0) {
												schedule_poll()
											}
										}
									}
						...
						session.socket.on("notify", function(msg) {
							...
							if (e == "recvmsg_by_msgid") {
								msgid = split_python(msg[1], " ", 7)[1];
								m_msgs_wait_clear.push(msgid);
								process_msg(decode_msg(msg[1]));
								schedule_poll_tail()
							}
						});
						session.messageInstantSend = function(peer, msg, cb) {
							...
							if (session.config_inst_msg_with_msgid === 1) {
								var msgid = null;
								if (typeof msg === "string") {
									msgid = JSON.parse(msg)["msgid"]
								}
								if (msgid) {
									body["messageID"] = msgid
								} else {
									body["messageID"] = get_time_in_ms() % 1e6 + session.m_msgid++ % 1e6
								}
							}
							session.call2("user_sendmsg", body, function(err, data) {
								...
								this.messageChannelSend = function(msg, f) {
									...
									if (session.config_inst_msg_with_msgid === 1) {
										...
										if (msgid) {
											...
											body["msgID"] = get_time_in_ms() % 1e6 + session.m_msgid++ % 1e6
										}
									}
									session.call2("channel_sendmsg", body, function(err, msg) {
										if (f) {
											f()
										}
									})
									...
									try {
										if (channel.onChannelAttrUpdated) {
											for (var k in r.attrs) {
												channel.onChannelAttrUpdated(k, r.attrs[k], "update")
											}
										}
									} catch (e) {
										console.error(e)
